...
import otus.homework.customview.manager.DataManager
...
class LineGraphView @JvmOverloads constructor(
    ...
    init {
        val itemsList = dataManager.readAndParse()

        val itemsByCategory = itemsList.groupBy(PayloadEntity::category)
        maxPrice = itemsByCategory.maxOf { it.value.sumBy(PayloadEntity::amount) }
        val realFirstDay = extractDayNumber(itemsList.minOf(PayloadEntity::time))
        firstDay =  if (realFirstDay >= 3) {
            realFirstDay - DAYS_BEFORE
        } else {
            1
        }
        lastDay = extractDayNumber(itemsList.maxOf(PayloadEntity::time))
    ...
    private fun initGraphItems(itemsByCategory: Map<String, List<PayloadEntity>>) {
        itemsByCategory.values.forEach { category ->
            ...
            items.add(LineGraphItem(pointsList,ColorHelper.getRandomColor(16)))
        }
    }
    ...
    private fun drawDateLabels(canvas: Canvas) {
        ...
        (firstDay..lastDay).forEach { dayNumber ->
            val offset = calculateDateLabelOffset(dayNumber)
            val fullDayNumber = formatDayNumber(dayNumber)
            canvas.drawText(
                fullDayNumber,
    ...
    private fun calculateDateLabelOffset(dayNumber: Int): Int {
    ...
    private fun formatDayNumber(dayNumber: Int): String {
